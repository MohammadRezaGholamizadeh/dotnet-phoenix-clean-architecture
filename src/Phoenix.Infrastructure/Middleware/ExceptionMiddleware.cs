...
internal class ExceptionMiddleware : IMiddleware
{
        ...
        RequestDelegate next)
    {
        ...
        {
            string email =
                _currentUser.GetUserEmail() is string userEmail
                ? userEmail : "Anonymous";
            ...
            Log.Error(
                $"{errorResult.Exception} Request failed with Status Code " +
                $"{context.Response.StatusCode} and Error Id {errorId}.");
            ...
            if (!response.HasStarted)
            {
                ...
                await response.WriteAsync(
                    _jsonSerializer.Serialize(errorResult));
            }
            else
            {
