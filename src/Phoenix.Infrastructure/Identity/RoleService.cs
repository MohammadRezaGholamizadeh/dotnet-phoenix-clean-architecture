...
internal class RoleService : IRoleService
{
    ...
    public async Task<bool> ExistsAsync(string roleName, string? excludeId) =>
        await _roleManager.FindByNameAsync(roleName)
            is ApplicationRoles existingRole
            && existingRole.Id != excludeId;

    public async Task<RoleDto> GetByIdAsync(string id) =>
        await _db.Set<ApplicationRoles>().SingleOrDefaultAsync(x => x.Id == id) is { } role
            ? role.Adapt<RoleDto>()
    ...
    public async Task<RoleDto> GetByIdWithPermissionsAsync(string roleId, CancellationToken cancellationToken)
    {
        ...
        role.Permissions = await _db.Set<ApplicationRoleClaim>()
            .Where(c => c.RoleId == roleId && c.ClaimType == PhoenixClaims.Permission)
    ...
    public async Task<string> CreateOrUpdateAsync(CreateOrUpdateRoleRequest request)
    {
        ...
        {
            // Create a new role.
            var role = new ApplicationRoles(request.Name, request.Description);
        ...
        else
        {
            ...
            if (PhoenixRoles.IsDefault(role.Name))
            {
    ...
    public async Task<string> UpdatePermissionsAsync(UpdateRolePermissionsRequest request, CancellationToken cancellationToken)
    {
        ...
        _ = role ?? throw new NotFoundException(_t["Role Not Found"]);
        if (role.Name == PhoenixRoles.Admin)
        {
        ...
        foreach (string permission in request.Permissions.Where(c => !currentClaims.Any(p => p.Value == c)))
        {
            ...
            {
                _db.Set<ApplicationRoleClaim>().Add(new ApplicationRoleClaim
                {
                    RoleId = role.Id,
